...
- **メーカー希望小売価格**: Yahoo!ショッピングAPIで取得した商品名でバンダイ ホビーサイトを検索し、返ってきた商品の`jancode`がスキャンしたJANコードと完全一致した場合のみ、バンダイ公式の希望小売価格を採用します（UI上は緑色の「公式照合済み」バッジ）。一致するものが無ければ`officialPrice`は`null`になり、UI上は「未確認」とだけ表示されます。**量販店の実売価格を定価として代用することはしません。**
...
- 商品名検索のヒット率は100%ではありません（JANコード完全一致で確認しているため、誤った価格が表示されることはありませんが、確認できず「未確認」表示になるケースがあります）
- バンダイ側の検索はスペースや`/`（スケール表記の1/144等）を含むキーワードだとフィルタが効かず全件返ってくる癖があるため、検索前にキーワード・スケール表記・グレード表記（HG/RG/MG/PG/SD/EG/FM）を除去しています
- 商品名クリーニング（[src/utils/itemName.ts](src/utils/itemName.ts)）では、メーカー名（バンダイスピリッツ／BANDAI SPIRITS）や末尾のシリーズ名（「機動戦士ガンダム〜」以降）もバンダイ側検索のヒット率を下げるノイズとして除去しています。また型式番号（MSZ-010等）付きで検索して0件だった場合、型式番号を除いて自動的に再検索するフォールバックもあります（[src/utils/bandaiHobby.ts](src/utils/bandaiHobby.ts)）
