...
- **メーカー希望小売価格**: Yahoo!ショッピングAPIで取得した商品名でバンダイ ホビーサイトを検索し、返ってきた商品の`jancode`がスキャンしたJANコードと完全一致した場合、バンダイ公式の希望小売価格を採用します（UI上は緑色の「公式照合済み」バッジ）。JANが一致しない場合でも、商品名が一意に一件だけ一致すれば代わりにその価格を採用します（バンダイスピリッツ移行に伴うJAN再発番等で、公式サイト側のJANデータ自体が古いまま更新されていない現行商品があるため）。ただし商品名が複数候補に一致してしまう場合（カラバリ・Ver.違い等で価格が異なる可能性があるため）は採用せず、`officialPrice`は`null`のまま「未確認」と表示します。**量販店の実売価格を定価として代用することはしません。**
...
- 商品名検索のヒット率は100%ではありません（JANコード完全一致または商品名の一意な一致で確認しているため、誤った価格が表示されることはほぼありませんが、確認できず「未確認」表示になるケースがあります）
- バンダイ側の検索はスペースや`/`（スケール表記の1/144等）を含むキーワードだとフィルタが効かず全件返ってくる癖があるため、検索前にキーワード・スケール表記・グレード表記（HG/RG/MG/PG/SD/EG/FM）を除去しています
- 商品名クリーニング（[src/utils/itemName.ts](src/utils/itemName.ts)）では、メーカー名（バンダイスピリッツ／BANDAI SPIRITS）や、商品名の後半に付いたシリーズ名（「機動戦士ガンダム〜」以降）もバンダイ側検索のヒット率を下げるノイズとして除去しています（出品者によってはシリーズ名が商品名の前半に来ることもあり、その場合は肝心のキット名まで巻き込んで消してしまうため、出現位置が後半にある場合のみ除去します）。また型式番号（MSZ-010等）付きで検索して0件だった場合、型式番号を除いて自動的に再検索するフォールバックもあります（[src/utils/bandaiHobby.ts](src/utils/bandaiHobby.ts)）。JANコードが完全一致しない場合は商品名一致にもフォールバックしますが、検索結果内で商品名が複数候補に一致してしまうときは（別バリエーションを誤って採用するリスクを避けるため）採用せず「未確認」のままにします
